...
void TokenStream::reduce(uint from)
{
    ...
    while (ns.size() > 0){
        ...
        if (xs[i].type == xop_PREFIX){
            ...
            xs[next].expr = applyPrefix(xs[i].op_tag, xs[next].expr);
            unlink(i);
            activate(xs[next].prev);

        }else if (xs[i].type == xop_POSTFIX){
            assert_debug(isExpr(xs[prev]));
            xs[prev].expr = applyPostfix(xs[i].op_tag, xs[prev].expr);
            unlink(i);
            ...
            assert_debug(isExpr(xs[next]));
            assert_debug(isExpr(xs[prev]));
            xs[prev].expr = applyInfix(xs[i].op_tag, xs[prev].expr, xs[next].expr);
            unlink(i);
            unlink(next);
        }
    }
